...
import { performances_uid_put, performances_uid_single } from "@/lib/zod/api/performances_uid";
...
export async function GET(
  ...
  const { id: performance_uid } = await params;
  ...
  const idValidationResult = performances_uid_single.safeParse({ performance_uid });
  ...
  const { data, error } = await supabase
    ...
    .eq("id", idValidationResult.data.performance_uid)
    .maybeSingle();
...
export async function PUT(
  ...
  const bodyValidationResult = performances_uid_put.safeParse(body);
...
export async function DELETE(
  ...
  const idValidationResult = performances_uid_single.safeParse({ member_uid });
